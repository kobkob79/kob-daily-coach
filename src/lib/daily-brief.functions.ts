/**
 * Server function that produces the KobiOS daily AI brief.
 *
...
const SYSTEM_PROMPT = `אתה KobiOS — מאמן AI בכיר בעברית המשלב תזונאי ספורט, מאמן כושר ורופא חינוכי.
...
החזר JSON תקין בלבד במבנה:
{
  ...
  "statusLine": "משפט קצר מאוד לסטטוס ('KobiOS עוקב אחריך בזמן אמת — עוד N מ״ל מים, X גרם חלבון')",
  "analysis": [
...
export const generateDailyBrief = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<DailyBrief> => {
    ...
    return {
      ...
      statusLine: String(parsed.statusLine ?? "").trim() || "KobiOS עוקב אחריך בזמן אמת.",
      analysis,
      supplementAnalysis,
