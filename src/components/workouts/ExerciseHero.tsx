 ...
 *
 * Media is resolved through the shared `active_workout` policy (demo video →
 * main image → thumbnail image, canonical roles only - see
 * `resolveExerciseMedia()`), the same resolver `ExerciseDetailsSheet` uses
 * for `exercise_details`. The DB `image_path` column is used as a last
 * resort before the placeholder. Loading shows a skeleton; a failed/absent
 * asset degrades to the placeholder without ever breaking the layout.
 */
...
export function ExerciseHero({
  ...
  const guideUnavailableLabel =
    "\u05d8\u05e8\u05dd \u05e9\u05d5\u05d9\u05db\u05d4 \u05ea\u05de\u05d5\u05e0\u05ea \u05de\u05d3\u05e8\u05d9\u05da \u05dc\u05ea\u05e8\u05d2\u05d9\u05dc";
  const { resolve, isPending } = useExerciseMedia({ exerciseId, exerciseName: name });
  ...
  // failure flag to the resolved item's identity (path + updatedAt, so an
  // in-place replace at the same path also counts as "new") and clear it
  // the moment that identity changes.
